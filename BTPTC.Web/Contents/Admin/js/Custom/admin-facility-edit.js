...
$(document).ready(function () {
    ...
    //$('#btnupdatefacility').click(function () {
    //    debugger
    //    if ($("#PhotoFacilityFrom").valid()) {
...
function initializeValidation() {
    ...
    jQuery("#EditFacilityFrom").validate({
        errorPlacement: function (error, element) {
            $("span[data-valmsg-for='" + $(element).attr("name") + "']").html($(error));
        },
        rules: {
...
function GetEditFacility() {
    $.ajax({
        type: 'GET',
        url: '/Admin/OurTown/GetEditFacility?Id=' + id,
        success: function (datas) {
            console.log(datas);
            if (datas.length > 0) {
                $('#Name').val(datas[0].Name)
                $('#GUID').val(datas[0].GUID)
                $.each(datas, function (i) {
                    ...
                    var imgpath = window.location.protocol + '//' + window.location.hostname + (window.location.port ? ':' + window.location.port : '') + "/Resources/Images/Facility/" + data.ImageGUID.replace(/-/g, '') + '.' + data.ImageExtension;
                    var imageadd = '<tr class="data-contact-person" id=' + rowCount + '>' +
                        ...
                        '<td class="center"><a id="Editdiv_' + rowCount + '" data-uniquecode=' + data.ImageGUID + '  class="Editlink" href="javascript:void(0)">Edit</a> | <a id="deldiv_' + rowCount + '"  class="linelink" href="javascript:void(0)">Delete</a></td>' +
                        //'<td class="center"><a id="deldiv_' + rowCount + '"  class="linelink" href="javascript:void(0)">Delete</a>|<a id="Editdiv_' + rowCount + '"  class="Editlink"  href="javascript:void(0)">Edit</a></td>' +
...
$(document).on('click', '#btnupdatefacility', function () {
  ...
  if ($("#PhotoFacilityFrom").valid()) {
    ...
    formData.append('ImageName', $('#LargeImageFileName').val());
